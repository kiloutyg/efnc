    ...
    public function exportCSV(EntityManagerInterface $em): Response
    {
        ...
        foreach ($efncs as $efnc) {
            $row = [
                ...
                $efnc->getRiskWeighting() ? $efnc->getRiskWeighting()->getRiskPriorityIndex() : '',
                $efnc->getProduct() ? $efnc->getProduct()->getCategory()->getName() : '',
                $efnc->getProduct() ? $efnc->getProduct()->getVersion()->getName() : '',
                $efnc->getProduct() ? $efnc->getProduct()->getColor()->getName() : '',
                $efnc->isArchived() !== null ? ($efnc->isArchived() ? 'true' : 'false') : '',
