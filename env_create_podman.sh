...
DATABASE_URL=mysql://root:\${MYSQL_ROOT_PASSWORD}@efnc-database-pod/\${MYSQL_DATABASE}?charset=utf8mb4&serverVersion=11.4.5-MariaDB&sslmode=verify_ca&sslrootcert=/etc/ssl/certs/ca-cert.pem
...
if [ "${APP_CONTEXT_SH}" == "prod" ]
    then
        ...
        envsubst < ./template.yml > ./efnc.yml;

        podman play kube --replace ./efnc.yml


        # Wait until the container is listed as running
        until [ "$(podman ps -q -f name=efnc-web)" ]; do
        echo "Waiting for container efnc-web to start..."
        sleep 1
        done

        # Wait until the webpack compiled successfully
        until podman logs --since 10s --tail 10 efnc-web 2>&1 | grep -q "webpack compiled successfully"; do
        ...
        # Create docker-compose.override.yml file to use the good entrypoint
        envsubst < ./template.yml > ./efnc.yml
        echo "Production efnc.yml file created successfully!";
        cat ./efnc.yml;
        ...
        # Create docker-compose.override.yml file to use the good entrypoint
        envsubst < ./template.yml > ./efnc.yml;
        echo "Development efnc.yml file created successfully!";
        cat ./efnc.yml;
fi